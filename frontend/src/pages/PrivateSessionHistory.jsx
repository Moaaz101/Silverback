...
export default function PrivateSessionHistoryPage() {
  ...
  const { loading, error, fetchPrivateFighters, fetchPrivateSessionHistory } = usePrivateSessions();
  ...
  useEffect(() => {
    loadSessions();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filters.fighterId, filters.startDate, filters.endDate]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Private Session History</h1>
          <p className="text-gray-500 mt-1">View all recorded private training sessions</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFilters({...filters, fighterId: e.target.value})}
          ...
          {(filters.fighterId || filters.startDate || filters.endDate) && (
            <button
              onClick={() => setFilters({ fighterId: '', startDate: '', endDate: '' })}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
              <div className="grid grid-cols-12 gap-4 text-sm font-semibold text-gray-700">
                ...
                <div className="col-span-3">Fighter</div>
                  ...
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-3">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900">{session.fighterName}</p>
                          {session.fighter && (
                            <p className="text-xs text-gray-500">
                              {session.fighter.sessionsLeft}/{session.fighter.totalSessionCount} left
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {/* Status */}
                    <div className="col-span-2">
                      <span className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(session.status)}`}>
                        {session.status === 'present' && 'Attended'}
                    ...
                    <div className="col-span-3">
                      {session.notes ? (
                        <div className="flex items-start space-x-2">
                          ...
                          <span className="text-sm text-gray-600 line-clamp-2">{session.notes}</span>
                        </div>
                      ) : (
            ...
            <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
              <p className="text-sm text-gray-600">
                Showing <span className="font-semibold">{sessions.length}</span> session{sessions.length !== 1 && 's'}
              </p>
            </div>
          </div>
        )}
